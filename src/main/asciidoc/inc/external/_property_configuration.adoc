...
The _idx_ suffixe defines the order of copying. Entries which _idx_ is an integer number are copied first in ascending order by the parsed value of _idx_. Entries which _idx_ is not an integer number are copied second in ascending alphabetical order by _idx_.
