...
| *docker.linksList*
| A comma separated list of links to other containers when starting a container. For example `<docker.linksLists>redis:db, saml:identity</docker.linksLists>` specifies a link using alias 'db' to the image named 'redis', and a link using alias 'identity' to the image named 'identity'
