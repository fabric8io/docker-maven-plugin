...
| *skipSSL*
| If the docker daemon is listening on port 2376 but is unsecured (hence, no SSL available), set this switch to true to enable communication on a not SSL-secured channel where the plugin expects an SSL-secured connection.
| `docker.skipSSL`
