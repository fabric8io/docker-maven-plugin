...
public class KeyStoreUtil {
    ...
    // Alias under which the private key entry is stored in the KeyStore built below. An alias is a
    // lookup name, not a credential; the protection password is supplied by the caller at runtime.
    private static final String DOCKER_KEY_ALIAS = "docker";
     ...
     * @param keyStorePassword protection password for the private key entry; the same value must be
     *                         used to read the key back. The KeyStore is in-memory only, so an
     *                         ephemeral random value generated by the caller is appropriate.
     * @return a keystore whose private key entry is protected with the given password
     *
     * @throws IOException is reading of the the PEMs failed
     * @throws GeneralSecurityException when the files in a wrong format
     */
    public static KeyStore createDockerKeyStore(String certPath, char[] keyStorePassword) throws IOException, GeneralSecurityException {
        ...
        keyStore.setKeyEntry(DOCKER_KEY_ALIAS, privKey, keyStorePassword, certs);
